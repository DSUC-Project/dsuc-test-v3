...
export function AcademyTrack() {
 ...
 <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-white p-4 border border-border-main shadow-sm">
 <Link to="/academy" className="inline-flex items-center text-sm font-bold uppercase tracking-widest text-text-main hover:-translate-x-1 transition-transform">
 <ArrowLeft className="w-5 h-5 mr-2" strokeWidth={3} /> Quay lại
 </Link>
 <div className="flex items-center gap-4">
 <div className="flex items-center gap-2 px-3 py-1.5 bg-primary text-primary-foreground border border-border-main shadow-sm text-text-main text-xs font-bold uppercase tracking-widest">
 ...
 <div className="flex items-center gap-2 px-3 py-1.5 bg-primary text-primary-foreground border border-border-main shadow-sm text-white text-xs font-bold uppercase tracking-widest">
 ...
 <div className="inline-flex px-4 py-1.5 font-bold tracking-widest uppercase mb-2 bg-surface text-text-main border border-border-main shadow-sm text-sm">
 Chuyên đề: {trackInfo.id}
 </div>
 <h1 className="text-5xl sm:text-7xl font-display font-bold tracking-tighter text-text-main py-2 uppercase" style={{ textShadow: '4px 4px 0 #111827', color: 'white' }}>
 {trackInfo.title}
 </h1>
 </header>

 <div className="relative mt-16 px-2 sm:px-4">
 <div className="absolute left-6 sm:left-8 top-8 bottom-8 w-2 bg-white border border-border-main shadow-sm">
 <div className="w-full bg-primary text-primary-foreground border border-border-main transition-all duration-1000" style={{ height: `${progressPercent}%` }} />
 </div>
 ...
 disabled={isLocked}
 >
 <div className={`relative flex-shrink-0 flex items-center justify-center w-12 h-12 border border-border-main transition-colors z-10 ${
 isLocked
 ? 'bg-gray-200'
 : isCompleted
 ...
 {isCompleted ? <CheckCircle2 size={24} strokeWidth={3} className="text-text-main" /> : <div className={`w-3 h-3 ${isLocked ? 'bg-gray-400' : 'bg-primary text-primary-foreground animate-ping'}`} />}
 </div>

 <div className={`relative flex-grow p-6 sm:p-8 border border-border-main transition-all overflow-hidden ${
 isCompleted
 ? 'bg-white shadow-sm'
 : isLocked
 ? 'bg-gray-100'
 ...
 <span className={`px-3 py-1 text-[10px] font-bold uppercase tracking-widest border border-border-main ${
 isCompleted ? 'bg-surface text-text-main shadow-sm' : isCurrent ? 'bg-primary text-primary-foreground shadow-sm' : 'bg-white text-gray-500'
 }`}>
 Bài {index + 1}
 </span>
 {isCurrent && (
 ...
 isCompleted ? 'text-text-main' : isLocked ? 'text-gray-500' : 'text-primary group-hover:text-primary'
 }`}>
 {lesson.title}
 </h3>
 </div>
 ...
 <span className={`flex items-center gap-2 ${isCompleted ? 'text-primary' : isLocked ? 'text-gray-500' : 'text-primary'}`}>
 <Clock size={16} strokeWidth={3} /> {lesson.minutes} PHÚT
 </span>
 {isLocked ? (
 <div className="p-2 border border-border-main bg-gray-200 text-gray-400">
 <Lock size={18} strokeWidth={3} />
 </div>
 ) : (
 <div className={`px-5 py-2 border border-border-main shadow-sm transition-transform ${isCompleted ? 'bg-white text-text-main group-hover:-translate-y-0.5' : 'bg-primary text-primary-foreground group-hover:-translate-y-0.5'}`}>
 ...
 <div className="text-text-main font-bold uppercase tracking-widest w-full text-center bg-white border border-border-main shadow-sm p-12">Không có bài học nào trong chuyên đề này.</div>
 )}
 </div>
 </div>
 </div>
