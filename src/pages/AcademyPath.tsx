...
import { ArrowLeft, ArrowRight, BookOpen, ChevronRight, Lock, Trophy } from 'lucide-react';
...
export function AcademyPath() {
 ...
 <div className="p-8 border border-border-main border-dashed bg-surface text-text-muted font-mono text-sm max-w-md mx-auto">
 {error || 'Path not found.'}
 </div>
 <div className="mt-8">
 <Link to="/academy"><ActionButton variant="primary">Return to Academy</ActionButton></Link>
 </div>
 </div>
 );
 }
 ...
 <div>
 <StatusBadge status={path.tag || path.difficulty} className="mb-6 inline-flex" />
 <h1 className="font-heading font-bold text-5xl md:text-7xl uppercase tracking-tighter leading-none mb-6">
 {path.title}
 </h1>
 <p className="text-lg text-text-muted max-w-2xl leading-relaxed">
 Complete this learning path progressively. Each course unlocks the next stage of your journey.
 </p>
 </div>
 </div>
 ...
 <p className="font-heading text-3xl font-bold">{path.course_count}</p>
 <p className="font-mono text-[10px] uppercase text-text-muted mt-1">Courses</p>
 </div>
 <div className="p-4 bg-surface text-center flex flex-col items-center justify-center min-w-[100px]">
 <p className="font-heading text-3xl font-bold">{path.practice_unit_count}</p>
 <p className="font-mono text-[10px] uppercase text-text-muted mt-1">Practice</p>
 </div>
 <div className="p-4 bg-surface text-center flex flex-col items-center justify-center min-w-[100px]">
 <p className="font-heading text-3xl font-bold text-emerald-500">{completedCourses}</p>
 <p className="font-mono text-[10px] uppercase text-text-muted mt-1">Done</p>
 </div>
 </div>
 </div>
 </section>
 ...
 {/* Courses Section */}
 <section>
 <SectionHeader title="Curriculum" subtitle="Courses included in this path." />

 {path.courses.length === 0 ? (
 <div className="p-12 text-center bg-surface border border-border-main border-dashed font-mono text-sm text-text-muted mt-8">
 ...
 <h3 className={`font-heading font-bold text-2xl md:text-3xl uppercase tracking-tight mb-3 truncate w-full ${locked ? 'text-text-muted' : 'text-text-main group-hover:text-primary transition-colors'}`}>
 {course.title}
 </h3>
 <p className={`text-sm leading-relaxed max-w-3xl line-clamp-2 ${locked ? 'text-text-muted/60' : 'text-text-muted'}`}>
 {course.description}
 </p>
 </div>
 ...
 <div>
 <div className="font-heading font-bold text-xl">{completed}<span className="text-sm font-sans font-normal text-text-muted">/{course.total_unit_count}</span></div>
 <div className="text-[10px] font-mono text-text-muted uppercase">Units</div>
 </div>
 <div>
 <div className="font-heading font-bold text-xl">{course.duration_hours}<span className="text-sm font-sans font-normal text-text-muted">h</span></div>
 <div className="text-[10px] font-mono text-text-muted uppercase">Duration</div>
 </div>
 </div>
 
 <div className="mt-auto hidden md:block">
 <span className={`inline-flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest transition-all ${locked ? 'text-transparent' : 'text-primary group-hover:gap-3'}`}>
