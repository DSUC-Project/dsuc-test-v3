...
import { FileText, Folder, Link as LinkIcon, Video, ExternalLink, Search } from 'lucide-react';
...
export function Resources() {
  ...
  const categories = ['All', ...Array.from(new Set(data.map(r => r.category || 'General')))];
  ...
  return (
    <div className="container mx-auto px-4 py-8 md:py-16 space-y-12">
      <SectionHeader title="Resources" subtitle="Documentation, guides, and tools for the community." />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredResources.map(resource => (
          <SoftBrutalCard key={resource.id} className="flex flex-col h-full group hover:bg-main-bg cursor-pointer transition-colors p-6" onClick={() => window.open(resource.url, '_blank')}>
            <div className="flex justify-between items-start mb-4">
              <div className="p-3 bg-surface border brutal-border group-hover:bg-primary/10 transition-colors">
                {getIcon(resource.type)}
              </div>
              {resource.type && <StatusBadge status={resource.type} className="bg-surface group-hover:bg-main-bg" />}
            </div>
            
            <h3 className="font-heading font-bold text-xl mb-2 group-hover:text-primary transition-colors">{resource.title || resource.name}</h3>
            <p className="text-sm text-text-muted mb-6 flex-grow">{resource.description}</p>
            ...
            <div className="flex items-center justify-between border-t brutal-border pt-4 mt-auto">
              <div className="flex flex-wrap gap-1">
                {resource.tags?.slice(0,2).map((tag: string) => (
                  <span key={tag} className="text-[10px] font-mono uppercase bg-surface px-1.5 py-0.5 border brutal-border text-text-muted">
                    {tag}
                  </span>
                ))}
              </div>
