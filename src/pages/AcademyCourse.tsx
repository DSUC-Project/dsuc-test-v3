...
export function AcademyCourse() {
 ...
 <div className="h-64 animate-pulse bg-surface border border-border-main" />
 </div>
 );
 }
 ...
 <StatusBadge status={course.path_title || 'Course'} className="inline-flex" />
 <span className="font-mono text-[10px] uppercase text-text-muted px-2 ">{difficultyLabel(course.difficulty)}</span>
 <span className="font-mono text-[10px] uppercase text-text-muted px-2 ">Stage {String(Math.max(1, course.track_level || 1)).padStart(2, '0')}</span>
 </div>
 ...
 {course.tags.map((tag) => (
 <span key={tag} className="font-mono text-[10px] uppercase bg-surface border border-border-main px-2 py-1 text-text-muted">
 {tag}
 </span>
 ))}
 </div>
 ...
 <p className="font-heading text-3xl font-bold">{course.duration_hours}<span className="text-sm font-sans font-normal text-text-muted">h</span></p>
 <p className="font-mono text-[10px] uppercase text-text-muted mt-1">Duration</p>
 </div>
 <div className="p-4 bg-surface text-center flex flex-col items-center justify-center min-w-[100px]">
 <p className="font-heading text-3xl font-bold">{course.module_count}</p>
 <p className="font-mono text-[10px] uppercase text-text-muted mt-1">Modules</p>
 </div>
 <div className="p-4 bg-surface text-center flex flex-col items-center justify-center min-w-[100px]">
 <p className="font-heading text-3xl font-bold text-emerald-500">{course.practice_unit_count}</p>
 <p className="font-mono text-[10px] uppercase text-text-muted mt-1">Exercises</p>
 </div>
 </div>
 </div>
 ...
 <div className="h-full bg-emerald-500 transition-all ease-out duration-1000" style={{ width: `${modulePercent}%` }} />
 </div>
 ...
 <div>
 <div className="font-heading font-bold text-xl">{module.learn_units.length}</div>
 <div className="font-mono text-[10px] uppercase text-text-muted">Theory</div>
 </div>
 <div>
 <div className="font-heading font-bold text-xl">{module.practice_units.length}</div>
 <div className="font-mono text-[10px] uppercase text-text-muted">Practice</div>
 </div>
 </div>
 </div>
 ...
 <User className="h-6 w-6 text-text-muted" aria-hidden="true" />
 </div>
 <div>
 <div className="font-heading text-lg font-bold uppercase mb-2">
 {course.instructor.name}
 </div>
 <p className="text-xs font-mono text-text-muted leading-relaxed">{course.instructor.bio}</p>
 </div>
 </div>
 </SoftBrutalCard>
 )}
 ...
 <div className={`text-[10px] font-mono ${percent === 100 ? 'text-emerald-500' : 'text-text-muted'} mt-1`}>
 {completed}/{total}
 </div>
 </div>
 </div>
...
function UnitLane({
 ...
 if (units.length === 0) {
 return (
 <div className="border border-border-main border-dashed p-6 flex flex-col items-center justify-center text-center bg-surface">
 <div className="text-[10px] font-mono uppercase bg-main-bg text-text-muted px-2 py-1 mb-3">
 {title}
 </div>
 <p className="text-xs text-text-muted/60 font-mono">No units added yet.</p>
 </div>
 );
 }
 ...
 <div className={`font-heading text-sm font-bold uppercase leading-tight ${locked ? 'text-text-muted' : done ? 'text-primary' : 'text-text-main group-hover:text-primary transition-colors'}`}>
 {unit.title}
 </div>
 </div>
 ...
 <Trophy className="h-4 w-4 text-emerald-500" aria-hidden="true" />
 ) : unit.section === 'practice' ? (
 <Code2 className="h-4 w-4 text-text-muted group-hover:text-primary transition-colors" aria-hidden="true" />
 ) : (
 <BookOpen className="h-4 w-4 text-text-muted group-hover:text-primary transition-colors" aria-hidden="true" />
 )}
 </div>
 </button>
 );
