  ...
  return (
    <div className="container mx-auto px-4 py-12 md:py-24 space-y-32">
      ...
      <section className="relative">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8 items-start">
          <div className="lg:col-span-7 flex flex-col items-start z-10">
            <div className="inline-flex items-center gap-2 px-3 py-1 border brutal-border mb-8 bg-surface uppercase font-mono text-xs tracking-widest text-text-muted">
              {currentUser ? <><Trophy size={14} className="text-primary" /> {firstName} is learning</> : 'Guest Preview'}
            </div>
            ...
            <p className="text-lg md:text-xl text-text-muted max-w-lg mb-10 leading-relaxed font-sans border-l-2 brutal-border pl-4">
              Learn Solana step-by-step, complete real-world exercises, and maintain your learning streak.
            </p>

            <div className="grid grid-cols-3 gap-4 w-full max-w-lg mb-8">
               <div className="p-4 border brutal-border bg-surface text-center">
                  <BookOpen className="w-5 h-5 mx-auto mb-2 text-primary" />
                  <p className="font-display text-2xl font-bold">{paths.length}</p>
                  <p className="font-mono text-[10px] uppercase text-text-muted mt-1">Paths</p>
               </div>
               <div className="p-4 border brutal-border bg-surface text-center">
                  <Boxes className="w-5 h-5 mx-auto mb-2 text-primary" />
                  <p className="font-display text-2xl font-bold">{totalCuratedUnits}</p>
                  <p className="font-mono text-[10px] uppercase text-text-muted mt-1">Total Units</p>
               </div>
               <div className="p-4 border brutal-border bg-surface text-center">
                  <Flame className="w-5 h-5 mx-auto mb-2 text-primary" />
                  <p className="font-display text-2xl font-bold">{totalCompletedUnits}</p>
                  <p className="font-mono text-[10px] uppercase text-text-muted mt-1">Completed</p>
               </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-5 relative w-full h-full min-h-[300px]">
            <div className="w-full bg-surface border brutal-border p-6 shadow-sm flex flex-col h-full">
              {currentUser ? (
                <div className="space-y-6 flex-1 flex flex-col">
                  <div className="flex items-start justify-between border-b brutal-border pb-6">
                    <div>
                      <StatusBadge status="Active Streak" className="mb-4 bg-primary/10 text-primary border-primary/20" />
                      <div className="font-display text-6xl font-bold leading-none">{currentStreak}</div>
                      <div className="mt-2 text-xs font-mono uppercase tracking-widest text-text-muted">{streakHeadline}</div>
                    </div>
                  ...
                  <div className="grid grid-cols-7 gap-2 my-auto">
                    {streakDays.map((day, idx) => (
                      <div key={idx} className="flex flex-col items-center gap-2">
                        ...
                        <div className={`flex h-10 w-full sm:h-12 items-center justify-center border brutal-border transition-all ${day.completed ? 'bg-primary text-main-bg' : 'bg-main-bg text-text-muted/30'} ${day.isToday && !day.completed ? 'border-primary border-dashed' : ''}`}>
                            {day.completed || day.isToday ? (
                              <Flame className="w-4 h-4" />
                            ) : null}
                        </div>
                        <span className={`text-[10px] font-bold font-mono ${day.isToday ? 'text-primary' : 'text-text-muted'}`}>
                  ...
                  <div className="grid grid-cols-2 gap-4 mt-auto">
                    <div className="border brutal-border bg-main-bg p-3">
                      <div className="text-[10px] font-mono uppercase tracking-widest text-text-muted">Current</div>
                      <div className="mt-1 font-bold">{currentStreak} Days</div>
                    </div>
                    <div className="border brutal-border bg-main-bg p-3">
                      <div className="text-[10px] font-mono uppercase tracking-widest text-text-muted">Last Active</div>
                      <div className="mt-1 font-bold text-sm truncate">{lastActivityLabel || 'Never'}</div>
                    </div>
                  </div>
                </div>
              ) : (
                <div className="flex flex-col h-full justify-center items-center text-center p-6 bg-main-bg border brutal-border border-dashed">
                  <StatusBadge status="Preview Mode" className="mb-6" />
                  <h3 className="font-heading text-xl font-bold mb-4">Track Your Learning</h3>
                  <p className="text-sm text-text-muted mb-8 max-w-sm">
                    Sign in to save your streak, track completion, and earn credentials.
                  </p>
                  <ActionButton variant="secondary">Login to Start</ActionButton>
                </div>
              )}
            </div>
          </div>
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {Array.from({ length: 3 }).map((_, index) => (
              <div key={index} className="h-64 animate-pulse bg-surface border brutal-border" />
            ))}
          </div>
        ) : error ? (
          <div className="border border-red-500/50 bg-red-500/10 text-red-500 px-6 py-4 text-sm font-mono text-center">
            {error}
          </div>
        ) : (
            ...
            {paths.map((path) => {
              ...
              return (
                <Link
                  ...
                  className="group flex flex-col bg-surface border brutal-border relative overflow-hidden transition-all hover:border-primary hover:shadow-[4px_4px_0_0_var(--shadow-brutal-color)] focus:outline-none"
                >
                  <div className="absolute top-0 left-0 w-full h-1 bg-main-bg border-b brutal-border">
                    <div 
                  ...
                  <div className="p-6 md:p-8 flex-1 flex flex-col">
                     <div className="flex justify-between items-start mb-6">
                        ...
                        <div className="text-right">
                           <div className="text-2xl font-display font-bold text-text-main group-hover:text-primary transition-colors">{progressPercent}%</div>
                        </div>
                     </div>
                     ...
                     <div className="grid grid-cols-2 gap-3 mb-8 w-full mt-auto">
                        <div className="border brutal-border bg-main-bg px-3 py-2">
                           <div className="font-bold">{path.course_count}</div>
                           <div className="text-[10px] font-mono text-text-muted uppercase mt-0.5">Courses</div>
                        </div>
                        <div className="border brutal-border bg-main-bg px-3 py-2">
                           <div className="font-bold">{path.total_unit_count}</div>
                           <div className="text-[10px] font-mono text-text-muted uppercase mt-0.5">Units</div>
                        </div>
                     </div>

                     <div className="flex items-center justify-between pt-4 border-t brutal-border text-sm font-bold">
                        <span className="text-text-muted capitalize">{path.difficulty}</span>
      ...
      <section>
        ...
        {loading ? (
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
               <div className="h-48 animate-pulse bg-surface border brutal-border" />
             </div>
        ) : communityTracks.length === 0 ? (
          <div className="p-12 text-center bg-surface border brutal-border border-dashed font-mono text-sm text-text-muted">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {communityTracks.map((track) => (
              <Link
                ...
                className="group flex flex-col bg-surface border brutal-border p-6 text-left transition-all hover:border-primary focus:outline-none"
              >
                ...
                <div className="flex flex-wrap items-center justify-between border-t brutal-border pt-4">
